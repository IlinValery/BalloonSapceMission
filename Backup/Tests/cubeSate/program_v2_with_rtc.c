...
ThreeWire myWire(10,9,11); // IO, SCLK, CE (DATA/CLK/RST)
...
void setup() {
  Serial.begin(9600);
  RTCIsAvalable = init_rtc(); delay(100);
...
String convertIntString(String source, int zeroCount){
  return source.substring(source.length() - zeroCount, source.length());
}


String getDateTimeForFile(const RtcDateTime& dt){
  String day, hours, mins, secs;

  day = "00" + String(dt.Day());
  hours = "00" + String(dt.Day());
  mins = "00" + String(dt.Minute());
  secs = "00" + String(dt.Second());

  day = convertIntString(day, 2);
  hours = convertIntString(hours, 2);
  mins = convertIntString(mins, 2);
  secs = convertIntString(secs, 2);
  return day + hours + mins + secs + ".csv";
}
